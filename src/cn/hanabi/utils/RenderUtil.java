...
public class RenderUtil {
    ...
    public static void drawCircle(double radius, double x, double lastTickX, double y, double lastTickY, double z, double lastTickZ, Color c) {
        ...
        double posX = lastTickX + (x - lastTickX) * (double) pTicks - (Minecraft.getMinecraft().getRenderManager()).getRenderPosX();
        double posY = lastTickY + (y - lastTickY) * (double) pTicks - (Minecraft.getMinecraft().getRenderManager()).getRenderPosY();
        double posZ = lastTickZ + (z - lastTickZ) * (double) pTicks - (Minecraft.getMinecraft().getRenderManager()).getRenderPosZ();
    ...
    public static void startDrawing() {
        ...
        GL11.glDisable(3553);
        GL11.glDisable(2929);
        (Minecraft.getMinecraft().entityRenderer).runSetupCameraTransform(Wrapper.getTimer().renderPartialTicks, 0);
    }
    ...
    public void drawCircle(Entity e, double radius, double x, double y, double z, double yOffset, Color c) {
        float pTicks = Wrapper.getTimer().renderPartialTicks;
        double posX = e.lastTickPosX + yOffset + (x - (e.lastTickPosX + yOffset)) * (double) pTicks - (Minecraft.getMinecraft().getRenderManager()).getRenderPosX();
        double posY = e.lastTickPosY + yOffset + (y - (e.lastTickPosY + yOffset)) * (double) pTicks - (Minecraft.getMinecraft().getRenderManager()).getRenderPosY();
        double posZ = e.lastTickPosZ + yOffset + (z - (e.lastTickPosZ + yOffset)) * (double) pTicks - (Minecraft.getMinecraft().getRenderManager()).getRenderPosZ();
        GlStateManager.pushMatrix();
        GlStateManager.disableTexture2D();
        GlStateManager.enableBlend();
        GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
