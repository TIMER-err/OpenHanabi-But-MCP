...
public class WorldUtil {
    public static List<EntityLivingBase> getLivingEntities() {
        return Arrays.asList(Minecraft.getMinecraft().theWorld.loadedEntityList.stream().filter((entity) -> entity instanceof EntityLivingBase).filter((entity) -> entity != Minecraft.getMinecraft().thePlayer).map((entity) -> (EntityLivingBase) entity).toArray((x$0) -> new EntityLivingBase[x$0]));
    }
