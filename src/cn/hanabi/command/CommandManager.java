...
public class CommandManager {
    ...
    public boolean executeCommand(@NotNull String string) {
        ...
        if (split.length == 0) {
            ...
            Command command = (Command) this.commands.stream().filter((cmd) -> cmd.equals(cmdName)).findFirst().orElse(null);
    ...
    public Collection autoComplete(@NotNull String currCmd) {
        ...
        Command currentCommand = split.length >= 1 ? (Command) this.commands.stream().filter((cmd) -> cmd.equals(split[0])).findFirst().orElse(null) : null;
        if (split.length < 2 && (currentCommand == null || !currCmd.endsWith(" "))) {
            if (split.length != 1) {
                ...
                return (Collection) ret.stream().map((str) -> "." + str).filter((str) -> ((String) str).toLowerCase().startsWith(currCmd.toLowerCase())).collect(Collectors.toList());
            }
