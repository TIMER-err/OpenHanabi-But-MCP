...
public class InvCleaner extends Mod {
      ...
      do {
         if (!var1.hasNext()) {
            ...
            for(int var3 = 0; var3 < var6; ++var3) {
               ItemStack itemStack1 = var5[var3];
               if (itemStack1 != null && itemStack1.getItem() instanceof ItemArmor && itemStack1 != input && ((ItemArmor)itemStack1.getItem()).armorType == ((ItemArmor)input.getItem()).armorType && InvUtils.getArmorScore(itemStack1) >= InvUtils.getArmorScore(input)) {
                  return false;
               }
            }
   ...
   private static double getSwordAttackDamage(ItemStack itemStack) {
      if (itemStack != null && itemStack.getItem() instanceof ItemSword) {
         ...
         return (double)(sword).getAttackDamage() + (double)EnchantmentHelper.getEnchantmentLevel(Enchantment.sharpness.effectId, itemStack) * 1.25 + (double)(EnchantmentHelper.getEnchantmentLevel(Enchantment.fireAspect.effectId, itemStack) * 1);
      } else {
         return 0.0;
      }
   }
   ...
   private static double getToolEffencly(ItemStack itemStack) {
      if (itemStack != null && itemStack.getItem() instanceof ItemTool) {
         ...
         return (float)EnchantmentHelper.getEnchantmentLevel(Enchantment.efficiency.effectId, itemStack) + sword.getEfficiencyOnProperMaterial();
      } else {
         return 0.0;
      }
   }
