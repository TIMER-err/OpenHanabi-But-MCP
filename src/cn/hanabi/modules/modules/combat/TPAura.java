...
public class TPAura extends Mod {
    ...
    private void onUpdate(EventUpdate event) {
        if (this.timer.isDelayComplete(KillAura.randomClickDelay(((Double) this.cps.getValue()).intValue(), ((Double) this.cps.getValue()).intValue() + 1)) && !this.thread.isAlive()) {
    ...
    private void doTPHit(EntityLivingBase entity) {
        ArrayList<Vec3> tpPath = PathUtils.computePath(mc.thePlayer, entity);
        tpPath.forEach((vec3) -> {
            this.path.add(vec3);
        ...
        List<Vec3> tpPath2 = Lists.reverse(tpPath);
        tpPath2.forEach((vec3) -> {
            mc.getNetHandler().addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(vec3.getX(), vec3.getY(), vec3.getZ(), true));
        });
    }
}
