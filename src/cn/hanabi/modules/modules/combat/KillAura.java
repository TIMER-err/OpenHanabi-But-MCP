...
public class KillAura extends Mod {
   ...
   private void update() {
      ...
      Iterator<EntityLivingBase> var1 = targets.iterator();
   ...
   public void rotation(float[] realLastRot) {
      ...
      if (this.rotationTimer.isDelayComplete((100.0 - (Double)smooth.getValueState()) * (double)RandomUtils.nextFloat(2.6F, 3.1F))) {
         ...
         this.lastRotations = RotationUtil.getNeededRotations(KillAura.AimUtil.getLocation(targets.get(this.index).getEntityBoundingBox().expand((Double)hitbox.getValue(), (Double)hitbox.getValue(), (Double)hitbox.getValue())), new Vec3(0.0, 0.0, 0.0));
      ...
      while(true) {
         ...
         do {
            if (!var2.hasNext()) {
               ...
               this.attackEntity();
               if (mc.playerController.getCurrentGameType() != GameType.SPECTATOR && !ModManager.getModule(KeepSprint.class).isEnabled()) {
   ...
   private List<EntityLivingBase> getTargets() {
      ...
      if (this.priority.isCurrentMode("Armor")) {
         ...
         stream = stream.sorted(Comparator.comparingDouble((o) -> {
            return getDistanceBetweenAngles(mc.thePlayer.rotationPitch, AimUtil.getRotations(o)[0]);
         }));
      ...
      if ((Boolean)this.targetHUD.getValueState()) {
         ...
         if (this.hudMode.isCurrentMode("Flat")) {
            ...
            if (this.nulltarget) {
               ...
               hpPercentage = diff / target.getMaxHealth();
            ...
            if (this.nulltarget) {
               this.healthBarWidth2 = RenderUtil.getAnimationStateSmooth(0.0, this.healthBarWidth2, 6.0F / (float)Minecraft.getDebugFPS());
               this.healthBarWidth = RenderUtil.getAnimationStateSmooth(0.0, this.healthBarWidth, 14.0F / (float)Minecraft.getDebugFPS());
               this.hudHeight = RenderUtil.getAnimationStateSmooth(0.0, this.hudHeight, 8.0F / (float)Minecraft.getDebugFPS());
            } else {
               this.healthBarWidth2 = AnimationUtil.moveUD((float)this.healthBarWidth2, (float)hpWidth, 6.0F / (float)Minecraft.getDebugFPS(), 3.0F / (float)Minecraft.getDebugFPS());
               this.healthBarWidth = RenderUtil.getAnimationStateSmooth(hpWidth, this.healthBarWidth, 14.0F / (float)Minecraft.getDebugFPS());
               this.hudHeight = RenderUtil.getAnimationStateSmooth(40.0, this.hudHeight, 8.0F / (float)Minecraft.getDebugFPS());
            }
            ...
            RenderUtil.drawGradientSideways(x, y + 37.0F, (double)x + this.healthBarWidth, y + 40.0F, (new Color(0, 81, 179)).getRGB(), healthColor);
   ...
   public void onRender(EventRender render) {
      ...
      if (target != null && (Boolean)this.esp.getValueState()) {
         ...
         if (this.EspMode.isCurrentMode("Cylinder")) {
            entity = target;
            x = entity.lastTickPosX + (entity.posX - entity.lastTickPosX) * (double)Wrapper.getTimer().renderPartialTicks - (mc.getRenderManager()).getRenderPosX();
            y = entity.lastTickPosY + (entity.posY - entity.lastTickPosY) * (double)Wrapper.getTimer().renderPartialTicks - (mc.getRenderManager()).getRenderPosY();
            z = entity.lastTickPosZ + (entity.posZ - entity.lastTickPosZ) * (double)Wrapper.getTimer().renderPartialTicks - (mc.getRenderManager()).getRenderPosZ();
   ...
   public void renderBox(EntityLivingBase entity) {
      double x = entity.lastTickPosX + (entity.posX - entity.lastTickPosX) * (double)Wrapper.getTimer().renderPartialTicks - (mc.getRenderManager()).getRenderPosX();
      double y = entity.lastTickPosY + (entity.posY - entity.lastTickPosY) * (double)Wrapper.getTimer().renderPartialTicks - (mc.getRenderManager()).getRenderPosY();
      double z = entity.lastTickPosZ + (entity.posZ - entity.lastTickPosZ) * (double)Wrapper.getTimer().renderPartialTicks - (mc.getRenderManager()).getRenderPosZ();
   ...
   public static void drawESP(EntityLivingBase entity, int color, EventRender e) {
      double x = entity.lastTickPosX + (entity.posX - entity.lastTickPosX) * (double)e.getPartialTicks() - (mc.getRenderManager()).getRenderPosX();
      double y = entity.lastTickPosY + (entity.posY - entity.lastTickPosY) * (double)e.getPartialTicks() - (mc.getRenderManager()).getRenderPosY();
      double z = entity.lastTickPosZ + (entity.posZ - entity.lastTickPosZ) * (double)e.getPartialTicks() - (mc.getRenderManager()).getRenderPosZ();
      float radius = 0.2F;
      int side = 6;
      GL11.glPushMatrix();
   ...
   static class AimUtil {
      ...
      public static Vec3 getLocation(AxisAlignedBB bb) {
         ...
         return new Vec3((Boolean)KillAura.resolverXZ.getValue() ? Objects.requireNonNull(resolve).xCoord : (bb.minX + (bb.maxX - bb.minX) * (reverse ? 1.0 - x : x)) * (mistake ? 1.0 + value * 0.1 : 1.0), (Boolean)KillAura.resolverY.getValue() ? Objects.requireNonNull(resolve).yCoord : bb.minY + (bb.maxY - bb.minY) * pitch, (Boolean)KillAura.resolverXZ.getValue() ? Objects.requireNonNull(resolve).zCoord : bb.minZ + (bb.maxZ - bb.minZ) * (reverse ? 1.0 - z : z) * (mistake ? 1.0 + value * 0.1 : 1.0));
      }
