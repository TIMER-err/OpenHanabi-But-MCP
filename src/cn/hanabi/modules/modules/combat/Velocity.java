   ...
   private void onPacket(EventPacket e) {
      ...
      if (modes.isCurrentMode("Cancel")) {
         ...
         if (e.getPacket() instanceof S12PacketEntityVelocity) {
            veloPacket = (S12PacketEntityVelocity)e.getPacket();
            if (veloPacket.getEntityID() == mc.thePlayer.getEntityId()) {
               (veloPacket).setX((int)((double)veloPacket.getMotionX() * (Double)this.x.getValue() / 100.0));
               (veloPacket).setY((int)((double)veloPacket.getMotionY() * (Double)this.y.getValue() / 100.0));
               (veloPacket).setZ((int)((double)veloPacket.getMotionZ() * (Double)this.x.getValue() / 100.0));
            }
         }

         if (e.getPacket() instanceof S27PacketExplosion) {
            S27PacketExplosion packet = (S27PacketExplosion)e.getPacket();
            (packet).setX(packet.func_149149_c() * ((Double)this.x.getValue()).floatValue() / 100.0F);
            (packet).setY(packet.func_149144_d() * ((Double)this.y.getValue()).floatValue() / 100.0F);
            (packet).setZ(packet.func_149147_e() * ((Double)this.x.getValue()).floatValue() / 100.0F);
         }
      }
