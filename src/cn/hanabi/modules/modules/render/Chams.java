   ...
   public void onRenderEntity(EventRenderLivingEntity event) {
      ...
      if (event.getEntity() instanceof EntityPlayer && event.getEntity() != mc.thePlayer && event.isPre()) {
         if (rainbow) {
            ...
            try {
               RendererLivingEntity renderer = (RendererLivingEntity) mc.getRenderManager().getEntityRenderObject(event.getEntity());
               if (renderer != null && mc.getRenderManager().renderEngine != null && renderer instanceof RendererLivingEntity) {
                  ...
                  (renderer).doRenderModel(event.getEntity(), event.getLimbSwing(), event.getLimbSwingAmount(), event.getAgeInTicks(), event.getRotationYawHead(), event.getRotationPitch(), event.getOffset());
                  GL11.glEnable(2929);
                  ...
                  GL11.glPopMatrix();
                  (renderer).doRenderLayers(event.getEntity(), event.getLimbSwing(), event.getLimbSwingAmount(), Wrapper.getTimer().renderPartialTicks, event.getAgeInTicks(), event.getRotationYawHead(), event.getRotationPitch(), event.getOffset());
                  GL11.glPopMatrix();
               }
