...
public class CaveFinder extends Mod {
    ...
    public void onRender(EventRender e) {
        BlockPos pos;
        IBlockState state;
        for (Iterator var2 = this.list.iterator(); var2.hasNext(); RenderUtil.drawSolidBlockESP((double) pos.getX() - (mc.getRenderManager()).getRenderPosX(), (double) pos.getY() - (mc.getRenderManager()).getRenderPosY(), (double) pos.getZ() - (mc.getRenderManager()).getRenderPosZ(), state.getBlock().getMaterial() == Material.lava ? 1.0F : 0.0F, 0.0F, state.getBlock().getMaterial() == Material.water ? 1.0F : 0.0F, 0.2F)) {
