...
public class HUD extends Mod {
    ...
    boolean isLag;
    private final SmoothAnimation hotbarAnimation = new SmoothAnimation(0.0, 200);
    ...
    public HUD() {
        ...
        this.setState(true);
        HashMap<Category, List<Mod>> moduleCategoryMap = new HashMap<>();

        Mod module;
        for (Iterator<Mod> var2 = ModManager.getModules().iterator(); var2.hasNext(); moduleCategoryMap.get(module.getCategory()).add(module)) {
            module = var2.next();
        ...
        moduleCategoryMap.entrySet().stream().sorted(Comparator.comparingInt((cat) -> cat.getKey().toString().hashCode())).forEach((cat) -> {
            Tab tab = new Tab(cat.getKey().toString());

            for (Mod o : cat.getValue()) {
                tab.addSubTab(new SubTab(o.getName(), (subTab) -> {
    ...
    private void render2D(EventRender2D event) {
        ...
        if ((Boolean) this.hotbar.getValueState() && mc.getRenderViewEntity() instanceof EntityPlayer && !mc.gameSettings.hideGUI) {
            ...
            this.hotbarAnimation.set(mc.thePlayer.inventory.currentItem * 20);
    ...
    public void drawArrowRect(float left, float top, float right, float bottom, int color) {
        ...
        bufferBuilder.pos(left - 5.0F, bottom, 0.0).endVertex();
        bufferBuilder.pos(right + 5.0F, bottom, 0.0).endVertex();
    ...
    public void renderPotionStatus(int width, int height) {
        ...
        for (var5 = mc.thePlayer.getActivePotionEffects().iterator(); var5.hasNext(); x -= 35) {
            ...
            float rectX = (float) (width - 120) + 110.0F * ((float) effect.getDuration() / ((float) (Integer) this.potionMaxDurations.get(effect.getPotionID())));
