...
public class AutoClicker extends Mod {
   ...
   public void onUpdate(EventUpdate event) {
      if (mc.thePlayer != null) {
         ...
         if ((mc.gameSettings.keyBindAttack).getPress() && mc.thePlayer.isUsingItem()) {
            this.swingItemNoPacket();
         }

         if ((mc.gameSettings.keyBindAttack).getPress() && !mc.thePlayer.isUsingItem() && this.left.isDelayComplete(1000.0 / (double)this.getDelay())) {
            if ((Boolean)this.jitter.getValueState()) {
               this.jitter(this.random);
            }

            (mc).setClickCounter(0);
            (mc).runCrinkMouse();
            isClicking = true;
            this.left.reset();
         }
      ...
      if (!this.isDone) {
         switch (this.timer) {
            case 0:
               (mc.gameSettings.keyBindUseItem).setPress(false);
               break;
            case 1:
            case 2:
               (mc.gameSettings.keyBindUseItem).setPress(true);
               break;
            case 3:
               (mc.gameSettings.keyBindUseItem).setPress(false);
               this.isDone = true;
   ...
   public void swingItemNoPacket() {
      if (!mc.thePlayer.isSwingInProgress || mc.thePlayer.swingProgressInt >= (mc.thePlayer).runGetArmSwingAnimationEnd() / 2 || mc.thePlayer.swingProgressInt < 0) {
