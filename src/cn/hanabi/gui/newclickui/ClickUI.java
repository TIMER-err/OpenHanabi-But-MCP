...
public class ClickUI extends GuiScreen {
   ...
   public void drawScreen(int mouseX, int mouseY, float par) {
      ...
      if (this.clickEffects.size() > 0) {
         Iterator clickEffectIterator2 = this.clickEffects.iterator();

         while(clickEffectIterator2.hasNext()) {
            ClickEffect clickEffect = (ClickEffect)clickEffectIterator2.next();
            clickEffect.draw();
            if (clickEffect.canRemove()) {
               clickEffectIterator2.remove();
            }
         }
      }
   ...
   protected void keyTyped(char typedChar, int keyCode) {
      ...
      if (keyCode == 14 && isSearching) {
         ...
         if (length != 0) {
            ...
            while(var7.hasNext()) {
               Panel panel2 = (Panel)var7.next();
               panel2.resetTranslate();
            }
         }
      }
   ...
   public void drawValue(float mouseX, float mouseY, float startX, float startY) {
      ...
      DoubleValue o2;
      for(var14 = this.modDoubleValue.iterator(); var14.hasNext(); valueY += o2.getLength()) {
         values2 = (Value)var14.next();
         if (doubleValueMap.containsKey(values2)) {
            o2 = (DoubleValue)doubleValueMap.get(values2);
         } else {
            o2 = new DoubleValue(values2);
            doubleValueMap.put(values2, o2);
         }

         o2.draw(startX, vstartY + valueY, mouseX, mouseY);
         o2.handleMouseinRender(mouseX, mouseY, 1);
      }

      BoolValue o3;
      for(var14 = this.modBooleanValue.iterator(); var14.hasNext(); valueY += o3.getLength()) {
         values2 = (Value)var14.next();
         if (booleanValueMap.containsKey(values2)) {
            o3 = (BoolValue)booleanValueMap.get(values2);
         } else {
            o3 = new BoolValue(values2);
            booleanValueMap.put(values2, o3);
         }

         o3.draw(startX, vstartY + valueY, mouseX, mouseY);
      }
