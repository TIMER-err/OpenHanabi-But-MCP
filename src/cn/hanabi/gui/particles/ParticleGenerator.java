...
public class ParticleGenerator {
   ...
   public void draw(int mouseX, int mouseY) {
      ...
      for (Particle particle : this.particles) {
         ...
         if (mouseOver) {
            this.particles.stream().filter((part) -> part.getX() > particle.getX() && part.getX() - particle.getX() < (float) range && particle.getX() - part.getX() < (float) range && (part.getY() > particle.getY() && part.getY() - particle.getY() < (float) range || particle.getY() > part.getY() && particle.getY() - part.getY() < (float) range)).forEach((connectable) -> {
