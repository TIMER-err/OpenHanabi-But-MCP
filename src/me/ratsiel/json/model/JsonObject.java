...
public class JsonObject extends JsonValue implements IListable<JsonValue> {
    ...
    public String toString() {
        ...
        this.loop((integer, jsonValue) -> {
            ((JsonValue) jsonValue).setIntend(this.getIntend() + 2);
            stringBuilder.append("\n");
            if (((Integer) integer) != this.size() - 1) {
    ...
    public JsonValue get(String key) {
        return !this.has(key) ? null : (JsonValue) this.values.stream().filter((jsonValue) -> {
            return ((JsonValue) jsonValue).getKey().equalsIgnoreCase(key);
        }).findFirst().get();
    }
    ...
    public boolean has(String key) {
        return this.values.stream().anyMatch((jsonValue) -> {
            return ((JsonValue) jsonValue).getKey() != null && ((JsonValue) jsonValue).getKey().equalsIgnoreCase(key);
        });
    }
