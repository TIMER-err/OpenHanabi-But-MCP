...
public class EntityRenderer implements IResourceManagerReloadListener {
            ...
            while (true) {
                while (var16.hasNext()) {
                    ...
                    if (axisalignedbb.isVecInside(vec3)) {
                        ...
                        if (d3 < d2 || d2 == 0.0) {
                            if (entity1 == entity.ridingEntity && !entity.isRiding()) {
                                if (d2 == 0.0) {
                                    this.pointedEntity = entity1;
            ...
            if (world != null) {
                ...
                if (worldColor.isEnabled()) {
                    ...
                    for (int ii = 0; ii < 256; ++ii) {
                        this.lightmapColors[ii] = a << 24 | r << 16 | g << 8 | b;
                    }
                } else {
                    for (i = 0; i < 256; ++i) {
    ...
    private void setupFog(int startCoords, float partialTicks) {
        ...
        GL11.glFogf(GL11.GL_FOG_COLOR, this.setFogColorBuffer(this.fogColorRed, this.fogColorGreen, this.fogColorBlue, 1.0F).get());
        GL11.glNormal3f(0.0F, -1.0F, 0.0F);
        GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
        Block block = ActiveRenderInfo.getBlockAtEntityViewpoint(this.mc.theWorld, entity, partialTicks);
        float f = -1.0F;
